        ...
        rightTop: rightTop
          ...
          flexDirectionEnd: flexDirectionEnd
				...
				<view v-for="(item, index) in content" :key="index" :class="{ active: isShow }" :style="{
					  ...
					  }" class="fab-item" @click="_onItemClick(index, item)" v-if="!item.hidden">
					<image v-if="item.icon.type=='img'" :src="baseurl+item.icon.icon" class="content-image" mode="widthFix" />
					<view v-else-if="(item.icon.type=='font')" :style="[{'color':item.color}]" :class="['cuIcon-recordfill']" mode="widthFix" style="font-size: 35rpx;"></view>
